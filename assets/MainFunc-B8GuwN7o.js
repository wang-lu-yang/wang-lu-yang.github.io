import{C as l}from"./CodeBlock-BeUO-N7C.js";import{C as r}from"./CodeLine-BM18u3Y-.js";import{T as o}from"./Tips-DNPYZNz3.js";import{a as d,H as m,o as g,c as u,h as e,A as n,g as s,f as a,u as T,F as x}from"./index-M27ktbsT.js";import"./Index-aTzL6mIO.js";const f={class:"overflw-x-auto custom-scrollbar unsolved-code"},E=["innerHTML"],M=d({__name:"MainFunc",emits:["updateOutlineList"],setup(k,{emit:i}){i("updateOutlineList",[{id:"#mainFunc_framework",title:"整体框架"},{id:"#mainFunc_notPlainText",title:"非纯文本元素"},{id:"#mainFunc_parseElement",title:"匹配注释、文档类型、标签"},{id:"#mainFunc_parseText",title:"匹配文本内容"},{id:"#mainFunc_isPlainText",title:"纯文本元素"}]);const p=m.highlight(`if (!isPlainTextElement(stackedTag) && stackedTag !== 'noscript') {
  text = text
    .replace(/<!--([sS]*?)-->/g, '$1') // #7298
...
`}),t[21]||(t[21]=e("h5",{id:"mainFunc_parseElement"},"· 匹配注释、文档类型、标签 ",-1)),t[22]||(t[22]=e("p",null," 匹配普通注释和条件注释的方式基本相同，都是先匹配到开始标志符之后，再去寻找结束标志符，若能匹配到就说明确实是注释类型的内容，接下来就需要忽略或者调用回调函数，然后向后移动游标并跳过剩余步骤。如果没有匹配到结束标志符，那就说明匹配到的开始标志符实际上只是文本内容（这一点与html的规则不同），因此不需要移动游标，而是把内容交给后续的文本解析过程处理。 ",-1)),t[23]||(t[23]=e("p",null," 匹配文档类型和结束标志的方式是类似的，都是完整匹配，若匹配到就忽略或进行相应处理。否则不做任何处理。需要特别说明的是，这里匹配到结束标签之后不会判断是否有对应的开始标签，而是直接传给parseEndTag函数处理，在上一篇中我们已经介绍到了这个函数，它会从堆栈中寻找开始标签，若找不到时只会对br和p标签进行特殊处理，如果是其它标签则会被直接忽略。也就是说在堆栈为空的情况下（因为vue2必须而且只能有一个根节点，所以只能是文档的开头或结尾），我们可以在模板内容中插入一些多余的结束标签而不导致报错，当然这并没有什么意义。 ",-1)),e("p",null,[t[4]||(t[4]=n(" 开始标签的匹配规则最为特殊，因为parseStartTag函数需要循环匹配属性，每次匹配到一个属性就向后移动一次游标，也就是说即使匹配不到完整的开始标签也会移动游标。比如 ")),s(r,{code:"<button disabled "}),t[5]||(t[5]=n(" ，这个button元素没有结束标志，startTagMatch匹配结果为空，所以并不会进行任何操作，但是parseStartTag函数又向后移动了游标，所以这段内容就会被忽略。所以说我们可以在模板内插入任意个未闭合的开始标签，这样并不会造成错误，但是也没有什么意义。如果我们需要将上述内容作为字符串插入模板，就必须要考虑这种被忽略的情况了（此时可以用html转码解决这一问题）。 "))]),s(l,{lang:"ts",path:"/src/compiler/parser/html-parser.ts",code:`const startTagMatch = parseStartTag()
if (startTagMatch) {
...
`}),t[28]||(t[28]=e("br",null,null,-1)),s(o,{type:"danger",icon:"QuestionFilled"},{title:a(()=>t[10]||(t[10]=[n("遗留问题")])),default:a(()=>[t[11]||(t[11]=e("p",null,"源码内解析纯文本元素处的这段代码，判断条件似乎永远不会成立？",-1)),e("pre",f,[e("code",{innerHTML:T(p)},null,8,E)])]),_:1})],64))}});export{M as default};
